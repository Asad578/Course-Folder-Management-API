...
import "./InstCourseOutline.css"; // You can create a similar CSS file like InstQuizContent

export default function InstLabContent() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/labs/${labId}/upload/`,
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/labs/${labId}/delete/`,
        {
          method: "PUT",
